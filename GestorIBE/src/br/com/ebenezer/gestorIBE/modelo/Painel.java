	...
	public boolean atualizar() {
		...
		if (conexao != null) {
			String sql = "update painel set " + "	codigo = ? ," + "	nome = ? ," + "	nascimento = ? ," + "	sexo = ? ,"
					+ "	telefone = ? ," + "	email = ? ," + "	cep = ? ," + "	endereco = ? ," + "	bairro = ? ,"
					+ "	cidade = ? ," + " uf = ? ," + " profissao = ? ," + "	escolaridade = ? ," + " estado_civil = ? ,"
					+ " conjuge = ? " + " where cod = ? ";
			try {
				PreparedStatement prepararSQL = conexao.prepareStatement(sql);
				prepararSQL.setInt(2, codigo);
				prepararSQL.setString(3, nome);
				prepararSQL.setString(4, nascimento);
				prepararSQL.setString(5, sexo);
				prepararSQL.setString(6, telefone);
				prepararSQL.setString(7, email);
				prepararSQL.setString(8, cep);
